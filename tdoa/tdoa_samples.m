function [doa_meters, doa_samples, doa_meters_2, doa_samples_2, correlation_value, correlation_value_interp] = ...
    tdoa_samples(signal1_complex, signal2_complex, num_samples_per_freq, num_samples_per_slice, us_slice, sample_rate, rx_distance_diff, ...
...
if interpol > 1
    signal12_complex = signal1_complex(num_samples_per_freq + delay1_interp + guard_interval : num_samples_per_freq + guard_interval + delay1_interp + us_slice - 1);
else
    signal12_complex = signal1_complex(num_samples_per_freq + delay1_native + guard_interval : num_samples_per_freq + guard_interval + delay1_native + us_slice - 1);
end
signal22_complex = signal2_complex(num_samples_per_freq + guard_interval : num_samples_per_freq + guard_interval   + us_slice - 1);
