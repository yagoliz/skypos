function [doa_meters, doa_samples, doa_meters_2, doa_samples_2, correlation_value, correlation_value_interp, mp_delay] = ...
    tdoa2_no_ref(signal1_complex, signal2_complex, num_samples_per_freq, num_samples_per_slice, sample_rate, ...
    max_lag, corr_type,  report_level, signal_bandwidth_khz_us, interpol, correct_multipath, mp_si)
...
if correct_multipath
    ...
    if length(peaks) > 1
        ...
        correlation_value = corr_normalized(real_peak);
        delay2_native = lags_normalized(real_peak);
        mp_delay = abs(min(locations) - real_peak); 
    else
        [correlation_value, idx2] = max(corr_signal_2);
...
if (interpol > 1)
    ...
    if correct_multipath
        ...
        [correlation_value_interp, ~] = max(corr_signal_2_interp(index-interpol-1:index+interpol+1));
        delay2_interp = lags2_interp(corr_signal_2_interp == correlation_value_interp(2));
        
    else
        [correlation_value_interp,idx2_interp] = max(corr_signal_2_interp);
...
doa_samples = delay2; % time difference of arrival without delays due to reception start time and ref transmitter (desc. above)
doa_samples_2 = delay2;
...
if report_level > 0
    ...
    disp(['raw delay2 (measure) (nativ/interp): ' int2str(delay2_native) ' / ' num2str(delay2_interp) ', reliability nativ: ' num2str(correlation_value)]);
    disp(' ');
